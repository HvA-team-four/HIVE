...
read input
                ...
                echo "
The script will automatically download the HIVE directory. Please specify the directory where you want HIVE to be downloaded to.

Specify directory as follow: /directory/directory/directory
"
				read input

				echo "Trying if directory $input exists." 
				cd "$input"
				...
				if [ "$?" = "0" ]; then
					...
					echo "Starting download to $input"	
					svn checkout https://github.com/HvA-team-four/HIVE.git/trunk
					echo "Files are downloaded"
					echo "Renaming folder to: HIVE"
					mv trunk HIVE
					echo "Folder renamed to: HIVE"
